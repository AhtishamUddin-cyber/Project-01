...
*Living document, updated at the end of every phase. Reflects the state of the project as of Phase 6's completion.*
...
| 7+ | Calibration/shadow-outcome tracking, external HTF/FLOW/SENTIMENT adapters, persistence, Streamlit integration | **Not started** |
...
**Not yet scoped or approved.** Recommended immediate next action: an independent audit of Phase 6 specifically (the pipeline orchestration, signal assembly, and scanner wiring above), ideally alongside the still-outstanding Phase 5 audit-fix sign-off noted at the top of this section. Beyond that, the remaining unscoped candidates are unchanged: `calibration/`/shadow-outcome tracking, the external HTF/FLOW/SENTIMENT adapters, persistence, and the Streamlit UI integration that consumes `OpportunityResult`. Waiting for an explicit next-phase prompt and approval before any further work begins.
